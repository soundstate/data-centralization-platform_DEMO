...
async def test_pokemon_correlation_query():
    ...
    try:
        ...
        response = await client.get_pokemon_species_by_generation(
            "generation-iii", limit=10
        )
        ...
        assert (
            "gen_species" in response.data
        ), f"gen_species key missing from response. Keys: {list(response.data.keys())}"
        assert (
            len(response.data["gen_species"]) == 10
        ), f"Expected 10 Pokemon species, got {len(response.data['gen_species'])}"
        ...
        print(
            f"✅ Test passed! Retrieved {len(response.data['gen_species'])} Generation III Pokemon species"
        )
        ...
        if "generation_info" in response.data and response.data["generation_info"]:
            ...
            total_count = (
                gen_info.get("pokemon_species", {}).get("aggregate", {}).get("count", 0)
            )
